...
public class AnadirReservaActivity extends Activity {
		...
		boton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(!(mensajeError = ComprobarFechas.comprobarFecha(fechaElegida, fechaHoy)).isEmpty())
					...
					ReservaInsertar reservaInsertar = new ReservaInsertar(fecha, 
														GestorPreferencias.getToken(AnadirReservaActivity.this),
														horaReserva,
														duracionMinutos,
														GestorPreferencias.getID(AnadirReservaActivity.this));
					String finHoraReserva = ComprobarFechas.formatHora(ComprobarFechas.horaFinal(tiempos, duracion, fechaElegida).getTimeInMillis());
					
					Reserva reserva = new Reserva(0, fecha, horaReserva, finHoraReserva,duracionMinutos,
													GestorPreferencias.getID(AnadirReservaActivity.this));
					new ConexionAsincronaInsertarReserva(
							AnadirReservaActivity.this, reservaInsertar, reserva).execute();
				}
	...
	private void mostrarToast(String msg) {
		Toast.makeText(AnadirReservaActivity.this, msg, Toast.LENGTH_LONG)
				.show();
	}
	...
	private class ConexionAsincronaInsertarReserva extends
		...
		public ConexionAsincronaInsertarReserva(Context c, ReservaInsertar r,
				Reserva reser) {
			this.contexto = c;
			res = r;
			reserva = reser;
		...
		protected ReponseInsertar doInBackground(String... params) {
			...
			Log.v("-AnadirReservaAsync-", "Json que envio:"+ dato);
			try {
				in = myDb.InsertarReservas(dato);
				if (in != null) {
				...
				if (result.getCode() >= Connection.NETWORK_ERROR)
					...
					if (result.getCode() == Connection.NETWORK_OK) {
						if (result.isInsertado()) {
							...
							new ReservaDatasource(contexto).insertarReserva(reserva);
							finish();
						}
